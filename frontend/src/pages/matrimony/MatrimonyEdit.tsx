...
export const MatrimonyEdit: React.FC = () => {
  ...
  const { uploadedImages, uploadedUrls, setUploadedImages, isUploading, uploadImages, removeImage } = useCloudinaryUpload({
    maxFiles: 5,
    folder: 'matrimony',
  });
  ...
  const fetchProfile = async () => {
    ...
    try {
      ...
      if (profile.photos && profile.photos.length > 0) {
        const existingPhotos = profile.photos.map((p: any) => ({
          url: p.url || p,
          publicId: p.publicId || p.split('/').pop()?.split('.')[0] || '',
        }));
  ...
  const onSubmit = async (data: MatrimonyProfileInput) => {
    ...
    try {
      // Format photos correctly - use existing photo objects with their publicIds
